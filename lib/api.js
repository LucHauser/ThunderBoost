...
// API for productHasVarieties

export async function getAllProductHasVarieties() {
    const response = await fetch(URL + "/productHasVarieties")
    if (!response.ok) {
        throw Error(response.statusText)
    }
    return await response.json()
...
export async function getProductHasVarietiesById(id) {
    const response = await fetch(URL + "/productHasVarieties/" + id )
    if (!response.ok) {
        throw Error(response.statusText)
    }
    return await response.json()
...
export async function addConnectProductWithVariety(productId, baseDataVarietyId, token) {

    console.log("IDs " + productId, baseDataVarietyId)
    const response = await fetch(URL + "/productHasVarieties", {
        method: "POST",
        headers: {
            "authorization": `Bearer ${token}`,
        ...
        body: JSON.stringify({productId: productId, baseDataVarietyId: baseDataVarietyId})
    })
    if (!response.ok) {
        throw Error(response.statusText)
    }
...
export async function deleteProductHasVarieties(id, token) {
...
export async function deleteConnectionsByProductId(productId, token) {
    const response = await fetch(URL + "/baseDataVariety?productId=" + productId, {
        method: "DELETE",
        headers: {
            "authorization": `Bearer ${token}`,
            "content-type": "application/json"
